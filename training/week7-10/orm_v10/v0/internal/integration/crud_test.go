...
import (
	...
	"gitlab.xchch.top/golang-group/go-101/training/week7-10/orm_v10/v0"
	"gitlab.xchch.top/golang-group/go-101/training/week7-10/orm_v10/v0/internal/test"
	"testing"
)
...
func (i *InsertTestSuite) TestInsert() {
	...
	testCases := []struct {
		name string
		i    *v0.Inserter[test.SimpleStruct]
		...
		{
			name:     "insert single",
			i:        v0.NewInserter[test.SimpleStruct](db).Values(test.NewSimpleStruct(14)),
			affected: 1,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			data, err := v0.NewSelector[test.SimpleStruct](db).
				Where(v0.C("Id").EQ(id)).Get(context.Background())
