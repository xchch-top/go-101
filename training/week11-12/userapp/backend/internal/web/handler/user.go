...
func NewUserHandler(us service2.UserService, sessMgr session.Manager) *UserHandler {
	return &UserHandler{
		service: us,
		sessMgr: sessMgr,
	}
...
func (h *UserHandler) SignUp(ctx *web.Context) {
	...
	if errors.Is(err, service2.ErrInvalidNewUser) {
	...
	if errors.Is(err, service2.ErrDuplicateEmail) {
		zap.L().Error("创建用户失败", zap.Error(err))
		_ = ctx.RespJSON(http.StatusBadRequest, Resp{
			Msg: "邮箱已被注册",
		})
