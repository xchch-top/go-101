...
import (
	...
	service2 "gitlab.xchch.top/golang-group/go-101/training/week11-12/userapp/backend/internal/service"
	"gitlab.xchch.top/golang-group/go-101/training/week11-12/userapp/backend/internal/service/mocks"
	"net/http"
	"testing"
	"time"
)
...
func TestUserHandler_Login(t *testing.T) {
	...
	testCases := []struct {
		...
		mock func() service2.UserService
...

		{
			...
			mock: func() service2.UserService {
				...
				us.EXPECT().Login(gomock.Any(), gomock.Any()).Return(entity.User{}, service2.ErrInvalidUserOrPassword)
				return us
			},
			ctx: func() *web.Context {
				body := bytes.NewBuffer([]byte(`{}`))
